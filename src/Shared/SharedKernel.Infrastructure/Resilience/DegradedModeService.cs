...
public sealed class RedisDegradedModeService : IDegradedModeService
{
    ...
    {
        var key = $"{DegradedModeKeyPrefix}{serviceName}";
    ...
    {
        var key = $"{DegradedModeKeyPrefix}{serviceName}";
        var configWithReason = config with { Reason = reason, ActivatedAt = DateTimeOffset.UtcNow };
        var value = JsonSerializer.Serialize(configWithReason);
        ...
        CancellationToken ct = default)
    {
        ...
        var metrics = await GetHealthMetricsAsync(serviceName, ct);
        ...
        var key = $"{HealthMetricsKeyPrefix}{serviceName}";
        var value = JsonSerializer.Serialize(updatedMetrics);
        await _cache.SetStringAsync(
            key,
            value,
        ...
        CancellationToken ct = default)
    {
        ...
        return JsonSerializer.Deserialize<ServiceHealthMetrics>(value) ?? new ServiceHealthMetrics
        {
            ServiceName = serviceName
        };
    }
        ...
        CancellationToken ct)
    {
        ...
        {
            var config = CreateDegradedModeConfig(serviceName, metrics);

    ...
    where TDbContext : DbContext
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!ModuleMatcher.IsMatch) return await next();
        // -------------------------------------------------------------
        ...
        try
        {
            ...
            if (_dbContext.Database.CurrentTransaction != null) return await next();
            ...
            return await strategy.ExecuteAsync(async () =>
            {
                ...
                var isFailure = false;
                if (response is Result result) isFailure = result.IsFailure;

                if (isFailure)
                {
                    // Do nothing, transaction disposes and rolls back
                    _logger.LogWarning(
                        "Transaction {TransactionId} for {CommandName} rolled back due to domain failure",
                        transaction.TransactionId, typeName);
                    return response;
                }

...
///     Purpose: Immediately revoke access when a user is banned/disabled in the identity provider,
...
///     against the identity provider's introspection endpoint on every request.
///     Performance Optimization:
///     - Caches introspection results in Redis for configurable duration (default: 30s)
///     - Uses token hash as cache key (never stores actual token)
///     - Short-circuits if introspection is disabled
///     2025 Security Standard: If a user is banned, they must be blocked within 30 seconds
///     across the entire system (Identity Mesh pattern).
/// </summary>
public sealed class TokenIntrospectionMiddleware
{
        ...
        IDistributedCache? cache = null)
    {
        ...
        // Perform introspection against identity provider
        ...
        IHttpClientFactory clientFactory)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                // Fail-open in case of identity provider unavailability (configurable)
            ...
            // Identity provider returns { "active": false } if token is revoked/invalid
        ...
        {
            _logger.LogError(ex, "Introspection request failed with exception");
            // Fail-open on exception (configurable - consider fail-closed for high-security environments)
            return new IntrospectionResult(true, "Introspection failed - fail-open");
        }
    }
    ...
    private static string ComputeTokenHash(string token)
    {
        ...
        return Convert.ToBase64String(bytes)[..16]; // Use first 16 chars for cache key
    }
}

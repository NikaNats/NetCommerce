...
public class CheckoutFlowLoadTests
{
    ...
    public void FlashSale_FullCheckoutFlow_ShouldMaintainConsistency()
    {
        ...
        var scenario = Scenario.Create("flash_sale_checkout", async context =>
            ...
            .WithLoadSimulations(
                // Ramp up users
                Simulation.RampingInject(10, TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(10)),
                // Sustain peak load
                Simulation.Inject(50, TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(30)),
                // Ramp down
                Simulation.RampingInject(5, TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(10))
            );
    ...
    public void ProductCatalog_SustainedReadLoad_ShouldBeFast()
    {
        ...
        var scenario = Scenario.Create("catalog_read_load", async context =>
            ...
            .WithoutWarmUp()
            .WithLoadSimulations(
                Simulation.KeepConstant(100, TimeSpan.FromSeconds(60))
            );
